...
func TestSpawner_CreateJobDeleteJob(t *testing.T) {
	...
	t.Run("starts and stops job services when jobs are added and removed", func(t *testing.T) {
		...
		delegateA := &delegate{jobTypeA, []job.Service{serviceA1, serviceA2}, 0, make(chan struct{}), offchainreporting.NewJobSpawnerDelegate(nil, orm, nil, nil, nil, nil, nil, nil, monitoringEndpoint)}
		...
		delegateB := &delegate{jobTypeB, []job.Service{serviceB1, serviceB2}, 0, make(chan struct{}), offchainreporting.NewJobSpawnerDelegate(nil, orm, nil, nil, nil, nil, nil, nil, monitoringEndpoint)}
	...
	t.Run("starts job services from the DB when .Start() is called", func(t *testing.T) {
		...
		orm := job.NewORM(db, config, pipeline.NewORM(db, config, eventBroadcaster), eventBroadcaster, &postgres.NullAdvisoryLocker{})
		defer orm.Close()
		delegateA := &delegate{jobTypeA, []job.Service{serviceA1, serviceA2}, 0, nil, offchainreporting.NewJobSpawnerDelegate(nil, orm, nil, nil, nil, nil, nil, nil, monitoringEndpoint)}
		spawner := job.NewSpawner(orm, config, map[job.Type]job.Delegate{
			jobTypeA: delegateA,
		})
	...
	t.Run("closes job services on 'delete_from_jobs' postgres event", func(t *testing.T) {
		...
		orm := job.NewORM(db, config, pipeline.NewORM(db, config, eventBroadcaster), eventBroadcaster, &postgres.NullAdvisoryLocker{})
		defer orm.Close()
		delegateA := &delegate{jobTypeA, []job.Service{serviceA1, serviceA2}, 0, nil, offchainreporting.NewJobSpawnerDelegate(nil, nil, nil, nil, nil, nil, nil, nil, monitoringEndpoint)}
		spawner := job.NewSpawner(orm, config, map[job.Type]job.Delegate{
			jobTypeA: delegateA,
		})
